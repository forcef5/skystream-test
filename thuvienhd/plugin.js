...
// Fshare: set domain to https://thuvienhd.top?fshare=email:password
...
function cleanT(t) { return t.replace(/\(\d{4}\)/g, "").replace(/(vietsub|thuyết minh|lồng tiếng|full hd|4k|bluray|[–—])/gi, " ").replace(/\s+/g, " ").trim(); }
...
async function parseListPage(html) {
    ...
    try {
        ...
        for (const art of articles) {
            ...
            items.push(new MultimediaItem({ title: title.trim(), url: href, posterUrl: poster || "", type: "movie", year: yearM ? parseInt(yearM[0]) : undefined }));
        }
...
const CATS = [
    ["Trending", "/trending/page/"], ["Phim Mới", "/recent/page/"],
    ["Phim Lẻ", "/genre/phim-le/page/"], ["Phim Bộ", "/genre/series/page/"],
    ["Thuyết Minh", "/genre/thuyet-minh-tieng-viet/page/"], ["Hành Động", "/genre/action/page/"],
    ["Kinh Dị", "/genre/horror/page/"], ["Hài", "/genre/comedy/page/"],
    ["Viễn Tưởng", "/genre/sci-fi/page/"], ["Hoạt Hình", "/genre/animation/page/"],
    ["Hàn Quốc", "/genre/korean/page/"], ["4K", "/genre/4k/page/"],
];
...
async function load(url, cb) {
    try {
        ...
        if (fshareEntries.length === 0) {
            ...
            if (dlLink) {
                try {
                    ...
                    for (const l of dlFs) { const h = l.getAttribute("href"); if (h && !fshareEntries.some(e => e.url === h)) fshareEntries.push({ url: h, fileName: h.split("/").pop(), fileSize: "" }); }
                } catch (_) { }
            }
        }
        ...
        const isSeries = hasFolders || fshareLinks.length > 1
            || tags.some(t => t.toLowerCase().includes("series") || t.toLowerCase().includes("phim bộ"))
            || doc.querySelector(".episodios") != null;
...
async function loadStreams(url, cb) {
    try {
        ...
        if (streams.length === 0) return cb({ success: false, errorCode: "NOT_FOUND", message: "Không tìm thấy stream. Kiểm tra Fshare VIP: ⚙️ Domains → https://thuvienhd.top?fshare=email:password" });
